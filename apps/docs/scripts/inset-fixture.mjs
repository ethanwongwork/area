...
export function buildInsetFixture(out, repo) {
  ...
  for (const ui of ['default', 'compact']) for (const dir of ['ltr', 'rtl']) for (const contrast of ['standard','more']) {
    const rows = ['xs','sm','md','lg','xl'].map(size => {
      ...
      h(Button,{size:buttonSize,variant:'soft',icon:globe,trailingIcon:chevron},'Text'),
      h(Button,{size:buttonSize,variant:'soft',trailingIcon:chevron},'Text'),
      h(Button,{size:buttonSize,variant:'soft',iconOnly:true,icon:globe,'aria-label':'Globe'}),
  ...
  copyFileSync(join(repo,'tests/fixtures/inset-before.css'),join(out,'inset-before.css'));
}
