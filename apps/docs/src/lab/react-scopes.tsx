...
export function ReactScopes({onChange}:{onChange:()=>void}) {
  ...
  const expected={...DEFAULT_AXES,theme:dark?'dark':'light',neutral:'warm',brand:override?'red':'green',density:'compact',radius:'pill'};
  return <section className="lab-report">
    ...
    <p id="react-requested" data-expected={JSON.stringify(expected)}>Expected: {expected.theme} / warm / {expected.brand} · compact · pill.</p>
    ...
    <Theme id="react-parent" value={{theme:dark?'dark':'light',neutral:'warm',brand:'green',density:'compact',radius:'pill'}}>
      <Theme id="react-child" value={override?{brand:'red'}:{}}><PortalSample/></Theme>
    </Theme>
  </section>;
}
